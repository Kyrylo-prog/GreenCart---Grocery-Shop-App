    ...
    return product &&(
        <div onClick={()=> {navigate(`/products/${product.category.toLowerCase()}/${product._id}`); scrollTo(0,0)}} className="border border-gray-500/20 rounded-md md:px-4 px-3 py-2 bg-white min-w-56 max-w-56 w-full">
            <div className="group cursor-pointer flex items-center justify-center px-2">
                <img className="group-hover:scale-105 transition max-w-26 md:max-w-36" src={product.image[0]} alt={getProductName(product)} />
            </div>
            <div className="text-gray-500/60 text-sm">
                <p>{getCategoryLabel(product.category)}</p>
                <p className="text-gray-700 font-medium text-lg truncate w-full">{getProductName(product)}</p>
                ...
                <div className="flex items-end justify-between mt-3">
                    ...
                    <div onClick={(e) => { e.stopPropagation();}} className="text-green-500">
                        {!cartItems[product._id] ? (
                            <button
                              className="flex items-center justify-center gap-1 bg-white border border-green-500 md:w-[80px] w-[64px] h-[34px] rounded text-green-500 hover:bg-green-500 hover:text-white transition cursor-pointer"
